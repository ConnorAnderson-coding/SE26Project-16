...
- 每个 `PENDING` 运行与一组不可更新的 `ClusteringRunInput` 在同一事务提交；后台执行只读取这组冻结输入。
...
### 2.2 `ClusteringRunInput`

`ClusteringRunInput` 是内部持久化的逐用户输入快照，不属于公开 API，也不是输出成员实体。每个有效用户在一个 run 中恰好一行，字段完整覆盖当前 Python `FeatureSample` 契约：

| 字段 | 语义与约束 |
| --- | --- |
| `inputId` | 不透明主键，最大 64 字符 |
| `runId` | 外键指向 `ClusteringRun`；run 删除时只级联删除其内部输入快照 |
| `userId` | Python 结果映射键；`UNIQUE(runId, userId)`，不保存姓名、角色或完整账号 |
| `sampleOrder` | 提交时的确定顺序；`UNIQUE(runId, sampleOrder)`，后台按此顺序一次加载 |
| `interestsJson` | 兴趣字符串数组 |
| `college`、`grade` | 当前 `FeatureSample`/Python 契约无损重建所需的可空类别特征；取提交时的冻结值 |
| `availableTimeJson` | 可参与时间字符串数组 |
| `signupCount`、`approvedSignupCount`、`favoriteCount`、`checkInCount`、`feedbackCount` | 非负计数；审核通过数不大于报名数 |
| `averageRating` | 可空；有值时在 `[1, 5]` |
| `categoryParticipationCountsJson` | 活动类别到非负参与次数的映射 |

所有列均不可更新。`ClusteringRunInput` 只保存无损执行所需的最小算法特征；在聚类快照模型中，`college`、`grade` 仅存在于该内部实体，不属于公开响应字段。该实体不保存姓名、密码或密码哈希、认证信息、角色或 authorities、联系方式、朋友实体或关系、完整 `UserAccount`、Python 地址或输出社区归属，也不存在到 `UserAccount` 的 `@ManyToOne`、`@OneToOne` 等实体关联。后台按 run 和 `sampleOrder` 一次读取，不逐行加载或回填实时用户实体；因此提交后实时资料变化以及应用重启后，仍能重建与提交时完全相同、顺序确定的 Python 请求。

公开 POST、运行详情、`latest`、`me` 和所有公开错误响应均不得序列化 `ClusteringRunInput` 或暴露其中的 `college`、`grade`。若未来移除这两个算法特征，必须通过新的 `featureSchemaVersion` 与对应 Python 契约版本实施；旧快照仍按原版本解释。
...
ClusteringRun (id, version, status)
  ├── ClusteringRunInput (runId, userId, sampleOrder) [冻结输入]
...
- 特征聚合和 K 值业务校验在数据库事务外完成；聚合结果复制为不可变输入集合。
- 创建 `ClusteringRun(PENDING)` 与写入该 run 的全部 `ClusteringRunInput` 使用同一个短事务，同时写入 `active_slot='GLOBAL'` 并 `flush`。任一快照失败会回滚整个 run；由 `uk_clustering_runs_active_slot` 原子拒绝第二个活动运行。
- 只有登记事务成功提交后，POST 才返回 `202 Accepted`。`202` 仅表示 run 与冻结输入已持久化接受，不代表 Python 已开始或成功。
- `PENDING -> RUNNING` 使用独立短事务和条件更新；不要在远程 Python 调用期间持有数据库事务或锁。
- 数据库中的 `PENDING` 是持久任务事实来源。调度器通过数据库锁原子领取最早记录；Spring 管理的单线程有界执行器只读取冻结输入，不重新聚合当前业务数据。
- Python 调用、响应校验均在数据库事务外执行；成功与失败各自使用独立短事务收口。
...
- 启动恢复保留 `PENDING`，随后由调度器重新领取；冻结输入不会因重启或业务数据变化而改变。
- 上次进程遗留的 `RUNNING` 不直接重放 Python，而是在独立事务中固定标记为 `FAILED`，记录 `EXECUTION_INTERRUPTED`、写入 `finishedAt` 并释放 active slot。
- 恢复操作幂等；终态不会被再次改写。
- 当前明确只支持单应用实例，不实现分布式 lease、心跳或 leader election。没有 Redis 或外部消息队列，不承诺消息队列级 exactly-once；数据库领取与状态机保证单实例内同一 run 不被重复正常执行。

## 10. 删除与保留策略

- MVP 不提供公开删除聚类运行的 API。
- 若后续增加管理员清理：删除某个运行可以只对派生数据采用 `ClusteringRun -> ClusteringRunInput/Community/CommunityMember` 的受控级联，且必须显式授权并在单事务执行。
