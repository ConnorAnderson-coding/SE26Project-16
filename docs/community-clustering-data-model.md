...
| `errorMessage` | `String` | `VARCHAR(1000)` | 是 | 仅失败时保存脱敏摘要；不得保存堆栈、凭据或内部地址 |
...
- `SUCCESS` 必须有 `startedAt`、`finishedAt`、`sampleCount` 和 `metricsJson`，且 `errorMessage` 应为 `null`。
- `metricsJson.pcaExplainedVarianceRatio` 固定为两个 `[0, 1]` 内有限数；零方差主成分保存 `0.0`，不得保存 `NaN` 或无穷大。
- `FAILED` 必须有 `finishedAt` 和脱敏 `errorMessage`，且不能成为 latest。
...
- 是否存在进行中的运行属于编排策略；MVP 建议通过应用层原子检查或数据库互斥约束避免多个 `PENDING`/`RUNNING` 任务并行，但不引入 Redis。
...
- `SUCCESS` 和 `FAILED` 是终态，不允许回退或被复用；重试必须创建新运行和新版本。
...
- 创建 `ClusteringRun(PENDING)` 使用短事务，提交后才向调用方返回 `202 Accepted`。
...
5. 写入 `metricsJson`、`sampleCount`、`finishedAt`，清空 `errorMessage`。
6. 最后将状态更新为 `SUCCESS` 并提交。
...
- 再使用独立短事务，把仍处于 `PENDING`/`RUNNING` 的运行更新为 `FAILED`，写入 `finishedAt` 和经过截断、脱敏的 `errorMessage`。
